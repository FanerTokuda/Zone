...
//    public ArrayList<Player> players = new ArrayList<>();
    ...
    // Chỉnh tnsm server (Chia cho tltnsm cho sư phụ)
    public long tnsmsp = 5;
    // Chỉnh tnsm server (nhân cho tltnsm cho đệ tử)
    public long tnsmdt = 1;
    // Chỉnh tỉ lệ đồ thần linh (tldothan/5000)
    public long tldothan = 2;
    // Chỉnh tỉ lệ đồ kích hoạt (tldokh/2000)
    public long tldokh = 1;
    // Chỉnh tỉ lệ máy dò 
    public long tlmaydo = 5;
    // Chỉnh tỉ lệ thức ăn
    public long tlthucan = 100;
    // Tỉ lệ tn ngũ hành sơn (Nhân)
    public long tnsmnhs = 1;
                ...
                if (itemPick.itemTemplateID == (short) 74) {
                    ...
                    if (itemPick.playerId == p.id || itemPick.playerId == -1) {
                        if (itemPick.item.template.id == 76 || itemPick.item.template.id == 188 || itemPick.item.template.id == 189 || itemPick.item.template.id == 190) { //TRUONG HOP ITEM LA VANG
                            int rdVang = Util.nextInt(10000000, 20000000);
                            ...
                            if (p.vang >= 2000000000) {// Thêm 4 thỏi vàng khi vàng hành trang quá 2 tỉ và reset
                                Item itemvang = ItemSell.getItemNotSell(457);
                                Item _itemvang = new Item(itemvang);
                                p.addItemToBag(_itemvang);
                                p.addItemToBag(_itemvang);
                                p.addItemToBag(_itemvang);
                                p.addItemToBag(_itemvang);
                                p.vang = 0;
                        ...
                        } else if (itemPick.item.template.id == 861 || itemPick.itemTemplateID == 77) { //TRUONG HOP ITEM LA  NGOC
                            int rdNgoc = Util.nextInt(1, 20);

                            itemsMap.remove(i);
                            Message m = new Message(-20);
                            m.writer().writeShort(itemMapId);
                            m.writer().writeUTF("");
                            m.writer().writeShort(rdNgoc);
                            m.writer().writeShort(rdNgoc);
                            p.session.sendMessage(m);
            ...
            if ((((Math.abs(p.x - mob.pointX) <= xSkill) && (Math.abs(p.y - mob.pointY) <= ySkill)) || (mobId == (byte) 70)) && !p.isTroi) {
                ...
                if (p.mp >= kiUse && !p.isdie && !mob.isDie && mob.hp > 0 && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown)) && (_timeNOW >= p.timeCanSkill)) {
                    ...
                    if (p.idSkillselect == 11) {
                        ...
                        long upSmTn = (long) (damage / tnsmsp);
...
//                            int idItemNotSell[] = {14,15,16,17,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225, 85, 993};
                                int idItemNotSell[] = {17, 18, 19, 20, 190, 190, 20, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 20, 85, 993};
                                int percentDrop = Util.nextInt(0, 20);
                                if (percentDrop >= 15) {
                                    Item itemscl = ItemSell.getItemNotSell(516); // ADD trực tiếp socola vào hành trang (Làm giống như nhặt từ đất lên)
                                    Item _itemscl = new Item(itemscl);
                                    p.sendAddchatYellow("Bạn đã nhặt được " + itemscl.template.name);
                                    p.addItemToBag(itemscl);
                                    p.updateItemBag();
                                    Service.gI().buyDone(p);
...
//                            int percentDropKichHoat = Util.nextInt(0, 5000);
                                ...
                                int perTA = Util.nextInt(0, 100);
//                                if (percentDropKichHoat < 5 && map.MapSetKichHoat()) { //ITEM ROT LA ITEM KICH HOAT
                                if (percentDropKichHoat < tldokh && map.MapSetKichHoat()) { //ITEM ROT LA ITEM KICH HOAT
...
//                                itemROI.itemTemplateID = (short)itemROI.itemMapID;
                                    ...
                                    itemsMap.addAll(itemDrop);
                                } else if (percentDrop < tlmaydo && map.MapCell() && p.useMayDoCapsule) { //ITEM ROT KHI DUNG RADA DO CAPSULE
                                    int idItemNotSell2[] = {17, 18, 19, 20, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 380};
                                    int id = Util.nextInt(0, 20);
                                    if (id >= 10) {
                                        id = 14;
                                    }
                                ...
                                } else if (percentDropItemGod < tldothan && map.MapCold()) {
...
//                                        }
                                ...
                                } else if (perTA < 6 && p.checkFullSetThan() && map.MapCold()) {
...
//                                }
                            ...
                            int percentDrop2 = Util.nextInt(0, 15);
                            if (mob.template.tempId != 0 && percentDrop2 < 3) {
                                int idItemNotSell2[] = {17, 18, 19, 20, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190};
            ...
            if (p.idSkillselect != 10) {
                ...
                if (p.mp >= kiUse && !p.isdie && !_bTarget.isdie && !p.isCharFreez && !p.isCharSleep && !p.isCharDCTT && !p.isCharBlind && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown)) && (_timeNOW >= p.timeCanSkill)) {
                    ...
                    if (p.getSetKichHoatFull() == (byte) 3 && skillPlayerUse.template.id == (byte) 1) {
                        ...
                        damage = damage * 2;
                    } else if (p.getSetKichHoatFull() == (byte) 5 && skillPlayerUse.template.id == (byte) 17) { //SET KICH HOAT ỐC TIÊU
                        damage = damage * 2;
                    }
                    //END CHECK SET KICH HOAT SONGOKU
                    // dame laze
...
//        Map map = player.map;
    ...
    public void refreshMob(int mobid) {
        try {
//            synchronized (this) {
            int rdSuperMob = Util.nextInt(0, 20); //random sieu quai
            Mob mob = getMob(mobid);
                    ...
                    for (Mob mob : mobs) {
                        if (Math.abs(player.x - mob.pointX) < templateSkillUse.dx && Math.abs(player.y - mob.pointY) < templateSkillUse.dy && !mob.isDie) {
                            ...
                            if (mob.isDie) {
                                ...
                                if (mob.template.tempId != 0) {
                                    int idItemNotSell[] = {17, 18, 19, 20, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190};
...
//                                                    }
    ...
    public void PetSendAttack(Player _player, Detu _detu, Mob _mob, Skill _skill) throws IOException {
        ...
        long upSmTn = (long) (damage / tnsmdt);
...
//        if(_detu.tiemNang > 0 && (_detu.hpGoc < _detu.getHpMpLimit() || _detu.mpGoc < _detu.getHpMpLimit() || _detu.damGoc < _detu.getDamLimit())) {
        ...
        if (_mob.isDie) {
            ArrayList<ItemMap> itemDrop = new ArrayList<>();
            if (_mob.template.tempId != 0) {
                int idItemNotSell[] = {17, 18, 19, 20, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 190};
                int percentDrop = Util.nextInt(0, 10);
                int percentDropItemGod = Util.nextInt(0, 5000);
